...
typedef struct
{
   ...
   pthread_t* nurse; //nurse threads
   pthread_t* doctor; //doctor threads
...
//Thread functions signatures (thread main dos pacientes)
...
void init_simulation(uint32_t np, uint32_t nN, uint32_t nd) // numero de pacientes, numero de nurses
{
   ...
   hd->pthr = new pthread_t[np]; //inicializar os pacientes
   hd->nurse = new pthread_t[nN]; //inicializar os infermeiros
   hd->doctor = new pthread_t[nd]; //inicializar os doutores
   
   //argumento para a thread pacientes
   /*O argumento attr aponta para uma estrutura pthread_attr_t cujo conteúdo é usado no momento da criação do 
   encadeamento para determinar os atributos do novo encadeamento; esta estrutura é inicializada usando pthread_attr_init (3) 
   e funções relacionadas. Se attr for NULL, o encadeamento será criado com atributos padrão.*/
   hd->pargs = new uint32_t[np]; // e para colocar na thread por ordem de prioridade
...
/* changes may be required to this function */
//DONE
void patient_wait_end_of_consultation(int id) // tempo da consulta
{
   check_valid_name(hd->all_patients[id].name);

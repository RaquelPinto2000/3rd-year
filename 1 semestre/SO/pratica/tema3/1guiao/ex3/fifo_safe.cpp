...
void fifoInit(Fifo* fifo)
{
    ...
    //reinicia todas as threads que estão esperando pela variável de condição cond . Nada acontece se nenhum tópico estiver esperando pelo cond 
    //Signall all waiting threads that fifo is not full
    // Sinaliza todos os threads em espera de que o fifo não está cheio
...
void fifoIn(Fifo* fifo,unsigned int id, unsigned int value)
{
    ...
    // Sinaliza todos os threads em espera de que fifo não está vazio
...
void fifoOut (Fifo* fifo,unsigned int * idp, unsigned int * valuep)
{
    ...
    *idp = fifo->slot[fifo->ri].id;
    ...
    //Signall all waiting threads that fifo is not full
    // Sinaliza todos os threads em espera de que fifo não está cheio
    cond_broadcast(&fifo->fifoNotFull);
    //Unlock acess to critical zone
    mutex_unlock(&fifo->accessCR);
}

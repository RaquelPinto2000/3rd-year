...
from flask_backend.data_processing import isClose, same_timestamp, isCongested
...
def get_all_cameras():
    ...
    print(cameras)
    result = cameras_schema.dump(cameras)
    return result
...
def get_all_ambulances_by_user():
    ambulances = Ambulance.query.all()
    result = ambulances_schema.dump(ambulances)
    return result
...
def can_login(email,password,user):
    if check_password_hash(user.password,password):
        return User.query.filter_by(email=email, password=user.password).first()
    else:
...
def get_accident_by(value, filter="all",quantity="All",order="Ascending",city=None):
    ...
    if filter == "congested":
        ...
        for accident in list(accidents):
            ...
            lng = float(accident.location["lng"])
            timestamp = accident.date
            loc = isCongested(location,(lat,lng))
            if (loc):
                count += 1
        return count
    ...
    if filter == "fire":
        if city == None:
            ...
            accident = Accident.query.filter_by(city=city)
        result = accidents_schema.dump(accident)
        result.sort(key=lambda x: x.get('fire'),reverse=(order=="Ascending"))
        if quantity == "Today":
            result = [accident for accident in result if
                        datetime.datetime.strptime(accident['date'].replace('T', " "), '%Y-%m-%d %H:%M:%S.%f')
                            .date() == current_time.date()]

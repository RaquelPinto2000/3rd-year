...
def severity_calc(n_people,velocity, ABS, airbag, overturned, all_seatbelts_on, fire_check):
...
def isClose(point1,point2):
    ...
    dist = distance.distance(point1,point2).km # obter distancia entre as duas localizações
...
def get_location_address(lat,lng):
    ...
    geolocator = Nominatim(user_agent="http") # obter geocoder  // mudamos aqui para criar acidentes
...
#verificar se o carro esta a arder
def check_burn(temp):
    if temp >= 115:
        return True
    else:
...
#Verificar a distancia entre acidentes
def isCongested(point1,point2):
